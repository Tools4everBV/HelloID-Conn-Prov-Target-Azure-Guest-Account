#####################################################
# HelloID-Conn-Prov-Target-Azure-Guest-Account-Disable
#
# Version: 1.1.0
#####################################################
...
try {
    if ($aRefMissing -eq $true) {
        if (-not($dryRun -eq $true)) {
            $auditLogs.Add([PSCustomObject]@{
                    # Action  = "DisableAccount" # Optional
                    Message = "aRef incomplete, cannot disable account"
                    IsError = $true
                })
        }
        else {
            Write-Warning "DryRun: aRef incomplete, cannot disable account. aRef object: $($aRef | ConvertTo-Json -Depth 10)" 
        }
    }
    elseif ($incompleteAccount -eq $true) {
        if (-not($dryRun -eq $true)) {
            $auditLogs.Add([PSCustomObject]@{
                    # Action  = "DisableAccount" # Optional
                    Message = "Account object incomplete, cannot disable account"
                    IsError = $true
                })
        }
        else {
            Write-Warning "DryRun: Account object incomplete, cannot disable account. Account object: $($account | ConvertTo-Json -Depth 10)" 
        }
    }
    else {
        ...
        if (-NOT($auditLogs.IsError -contains $true)) {
            ...
            try {
                ...
                if (-not($dryRun -eq $true)) {
                    Write-Verbose "Disabling Azure AD account '$($currentAccount.userPrincipalName) ($($currentAccount.id))'. Account object: $($account | ConvertTo-Json -Depth 10)"

                    $disableAccountResponse = $null
                    $disableAccountResponse = Invoke-RestMethod @splatWebRequest -Verbose:$false
                    ...
                    $auditLogs.Add([PSCustomObject]@{
                            # Action  = "DisableAccount" # Optional
                            Message = "Successfully disabled Azure AD account '$($currentAccount.userPrincipalName) ($($currentAccount.id))'"
                            IsError = $false
                        })
                }
                else {
                    Write-Warning "DryRun: Would disable Azure AD account '$($currentAccount.userPrincipalName) ($($currentAccount.id))'. Account object: $($account | ConvertTo-Json -Depth 10)"
                }
            }
            catch {
                # Clean up error variables
                ...
                $auditLogs.Add([PSCustomObject]@{
                        # Action  = "DisableAccount" # Optional
                        Message = "Error disabling Azure AD account '$($currentAccount.userPrincipalName) ($($currentAccount.id))'. Account object: $($account | ConvertTo-Json -Depth 10). Error Message: $auditErrorMessage"
                        IsError = $true
                    })
            }
        }
