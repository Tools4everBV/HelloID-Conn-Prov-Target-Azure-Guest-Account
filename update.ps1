#####################################################
# HelloID-Conn-Prov-Target-Azure-Guest-Account-Update
#
# Version: 1.1.0
#####################################################
...
$account = [PSCustomObject]@{
    displayName = $p.DisplayName -replace '( \().*$', '' # Remove everyting after ' (' to not include employeeID in displayname
    givenName   = $p.Name.NickName
    surname     = $p.Name.FamilyName
    #otherMails =   @($p.Contact.Personal.Email);
}

# Check if required fields are available
$incompleteAccount = $false
$requiredFields = @('displayName', 'givenName', 'surname')
...
try {
    if ($aRefMissing -eq $true) {
        if (-not($dryRun -eq $true)) {
            $auditLogs.Add([PSCustomObject]@{
                    ...
                    Message = "aRef incomplete, cannot update account"
                    IsError = $true
                })
        }
        else {
            Write-Warning "DryRun: aRef incomplete, cannot update account. aRef object: $($aRef | ConvertTo-Json -Depth 10)" 
        }
    }
    elseif ($incompleteAccount -eq $true) {
        if (-not($dryRun -eq $true)) {
            $auditLogs.Add([PSCustomObject]@{
                    # Action  = "UpdateAccount" # Optional
                    Message = "Account object incomplete, cannot update account"
                    IsError = $true
                })
        }
        else {
            Write-Warning "DryRun: Account object incomplete, cannot update account. Account object: $($account | ConvertTo-Json -Depth 10)" 
        }
    }
    else {
        ...
        if (-NOT($auditLogs.IsError -contains $true)) {
            ...
            try {
                ...
                if (-not($dryRun -eq $true)) {
                    Write-Verbose "Updating Azure AD account '$($currentAccount.userPrincipalName) ($($currentAccount.id))'. Account object: $($account | ConvertTo-Json -Depth 10)"
                    ...
                    $auditLogs.Add([PSCustomObject]@{
                            # Action  = "UpdateAccount" # Optional
                            Message = "Successfully updated Azure AD account '$($currentAccount.userPrincipalName) ($($currentAccount.id))'"
                            IsError = $false
                        })
                }
                else {
                    Write-Warning "DryRun: Would update Azure AD account '$($currentAccount.userPrincipalName) ($($currentAccount.id))'. Account object: $($account | ConvertTo-Json -Depth 10)"
                }
            }
            catch {
                # Clean up error variables
                ...
                $auditLogs.Add([PSCustomObject]@{
                        # Action  = "UpdateAccount" # Optional
                        Message = "Error updating Azure AD account '$($currentAccount.userPrincipalName) ($($currentAccount.id))'. Account object: $($account | ConvertTo-Json -Depth 10). Error Message: $auditErrorMessage"
                        IsError = $true
                    })
            }
        }
